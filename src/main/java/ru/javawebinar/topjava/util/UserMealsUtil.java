...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed>  getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for(UserMeal meal : mealList) {
            ...
            caloriesSumPerDay.put(mealDate, caloriesSumPerDay.getOrDefault(mealDate, 0) + calories);
        }
        ...
        for(UserMeal meal : mealList) {
            if(TimeUtil.isBetween(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                LocalDate mealDate = meal.getDateTime().toLocalDate();
               mealsWithExceed.add(createFromMeal(meal, caloriesSumPerDay.get(mealDate) > caloriesPerDay));
            }
        }
        return mealsWithExceed ;
    }
    private static UserMealWithExceed createFromMeal(UserMeal meal, boolean exceed) {
